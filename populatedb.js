...
var UserMeasurement = require('./models/usermeasurement')
...
function patientCreate(first_name, family_name, d_birth, cb) {
    ...
    console.log('Новый пациент: ' + patient);
    patients.push(patient)
    cb(null, patient)
    });
}

function specialtyCreate(title, cb) {
    var specialty = new Specialty({ title: title });
    specialty.save(function (err) {
    if (err) {
        cb(err, null);
        return;
    }
    console.log('Новая специальность: ' + specialty);
    specialties.push(specialty)
    cb(null, genre);
    });
}

function measureCreate(title, summary, isbn, patient, specialty, cb) {
    measuredetail = { 
        title: title,
        summary: summary,
        patient: patient,
    ...
    if (specialty != false) measuredetail.specialty = specialty
    
    var measure = new Measure(measuredetail);    
    measure.save(function (err) {
    if (err) {
        cb(err, null)
        return
    }
    console.log('Новое измерение: ' + measure);
